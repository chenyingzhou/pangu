...
class BalanceBill : BaseEntity() {
    ...
    @Column(nullable = false, columnDefinition = "varchar(255) DEFAULT '' COMMENT '钱包明细编号'")
    var billNo = ""
    ...
    @Enumerated(EnumType.STRING)
    @Column(name = "`status`", nullable = false, columnDefinition = "varchar(255) DEFAULT '' COMMENT '状态'")
    var status = Status.INIT

    @Column(nullable = false, columnDefinition = "bigint DEFAULT '0' COMMENT '发生金额'")
    var amount = 0L

    @Column(name = "`before`", nullable = false, columnDefinition = "bigint DEFAULT '0' COMMENT '发生前金额'")
    var before = 0L

    @Column(name = "`after`", nullable = false, columnDefinition = "bigint DEFAULT '0' COMMENT '发生后金额'")
    var after = 0L

    @Column(nullable = false, columnDefinition = "varchar(255) DEFAULT '' COMMENT '支付方式(仅用于前端展示)'")
    var payType = ""
